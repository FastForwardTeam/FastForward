...
if (args.has('target') && args.has('referer')) {
  ...
  const a = span.querySelector('a'),
    closetimer = () =>
      brws.storage.sync.get(['crowd_close_delay'], res =>
        timer('crowdCloseTimer', res.crowd_close_delay, true, () => {
          brws.runtime.sendMessage({ type: 'close-tab' })
        })
      )
  a.textContent = args.get('target')
  a.href = args.has('referer')
  ...
  document.getElementById('ignore').href =
    args.get('referer') + '#ignoreCrowdBypass'
  brws.storage.sync.get(
    ['crowd_open_delay'],
    res =>
      (opentimer = timer(
        'crowdBypassedTimer',
        res.crowd_open_delay,
        true,
        () => {
