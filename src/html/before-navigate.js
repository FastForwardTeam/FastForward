...
if (args.has('target')) {
  ...
  const when_safe = () => {
    ...
    a.href =
      referer && referer != 'tracker'
        ? 'https://fastforward.team/navigate' + location.search
        : args.get('target')
    brws.storage.sync.get(
      ['navigation_delay', 'no_instant_navigation_trackers'],
      res => {
        if (
          res.navigation_delay == 0 ||
          (referer == 'tracker' &&
            res.no_instant_navigation_trackers !== 'true')
        ) {
          document.querySelector('div').innerHTML = '<p></p>'
          document.querySelector('p').textContent = brws.i18n
            .getMessage('beforeNavigateInstant')
            .replace('%', args.get('target'))
          setTimeout(() => (location.href = a.href), 10)
        } else {
          timer(
            'beforeNavigateTimer',
            res.navigation_delay,
            true,
