...
brws.runtime.onInstalled.addListener(details => {
  ...
  chrome.storage.local.get({ ff_clipboard: '{}' }, function (data) {
    chrome.storage.local.set(
      { ff_clipboard: data.ff_clipboard },
      function () {}
    )
  })
})
      ...
      xhr.onerror = () => {
        ...
        xhr.onload = () => {
          ...
          if (latestCommit == upstreamCommit) {
            ...
            xhr.open(
              'GET',
              'https://cdn.jsdelivr.net/gh/FastForwardTeam/FastForward@' +
                upstreamCommit +
  ...
  refreshInjectionScript = () => {
    ...
    if (enabled) {
      injectionScript = (upstreamInjectionScript + '\n' + userScript)
        .split('FAST_FORWARD_INTERNAL_VERSION')
        .join('10')
        .split('FAST_FORWARD_EXTERNAL_VERSION')
        .join(extension_version)
        .split('FAST_FORWARD_INJECTION_VERSION')
  ...
  switch (req.type) {
    ...
    case 'crowd-contribute':
      if (crowdEnabled) {
        ...
        xhr.open(
          'POST',
          'https://crowd.fastforward.team/crowd/contribute_v1',
          true
        )
...
brws.webRequest.onBeforeRequest.addListener(
  ...
  { types: ['main_frame'], urls: ['*://fastforward.team/firstrun?0'] },
  ['blocking']
)
brws.webRequest.onBeforeRequest.addListener(
  details => {
  ...
  { types: ['main_frame'], urls: ['*://fastforward.team/firstrun?1'] },
  ['blocking']
)
...
brws.webRequest.onBeforeRequest.addListener(
  ...
  },
  {
    types: ['main_frame'],
    urls: [
      '*://fastforward.team/bypassed?target=*',
      '*://fastforward.team/bypassed?target=*&referer=*'
    ]
  },
  ['blocking']
)
...
brws.webRequest.onBeforeRequest.addListener(
  ...
  {
    types: ['main_frame'],
    urls: ['https://fastforward.team/crowd-bypassed?*']
  },
  ['blocking']
)
...
brws.webRequest.onBeforeRequest.addListener(
  ...
  { types: ['main_frame'], urls: ['https://fastforward.team/options*'] },
  ['blocking']
)
...
brws.webRequest.onBeforeRequest.addListener(
  ...
  { types: ['main_frame'], urls: ['*://fastforward.team/navigate?target=*'] },
  ['blocking']
)
