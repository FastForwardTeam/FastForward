...
const ODP = (t, p, o) => {
  ...
  unsafelyAssignWithReferer = (target, referer) => {
    ...
    location.href =
      'https://fastforward.team/navigate?target=' +
      encodeURIComponent(target) +
      '&referer=' +
      encodeURIComponent(referer)
  },
...
;(domainBypass = (domain, f) => {
  let FastForward_definition = new FastForwardBypassDefinition({
    url: domain,
    is_regex: typeof domain !== 'string' && 'test' in domain,
    execution: f
  })
  if (bypass_definitions.has(domain.toString())) {
    FastForward_definition = bypass_definitions.get(domain.toString())
  }
  FastForward_definition.update({
    url: domain,
    is_regex: typeof domain !== 'string' && 'test' in domain,
    execution: f
  })
...
class ffClipboard {
  ...
  static set (id, value) {
    ...
    if (ffClipboardObj) {
      ...
      let message = {
        type: 'ffclipboardSet',
        text: JSON.stringify(ffClipboardObj)
      }
...
domainBypass(/earnme\.club|usanewstoday\.club/, () => {
  awaitElement('#tp-snp2', a => {
    a.click()
  })
})
...
hrefBypass(/.*syosetu\.org\/\?mode=url_jump&url=.+/, () => {
  safelyNavigate(
    decodeURIComponent(
      document.URL.match(/.*syosetu\.org\/\?mode=url_jump&url=(.+)/)[1]
    )
  )
})
...
domainBypass('boost.ink', () =>
  fetch(location.href)
    .then(r => r.text())
    .then(html => safelyNavigate(atob(html.split('kekw="')[1].split('"')[0])))
)
...
hrefBypass(myDramaListRegex, () => {
  safelyNavigate(
    decodeURIComponent(
      document.URL.split(/\bmydramalist\.com\/redirect\?q=/)[1]
    )
  )
})
...
hrefBypass(
  ...
  () => ifElement('a.downloadAPK.dapk_b[href]', a => safelyAssign(a.href))
)
...
domainBypass('fastforward.team', () => {
  ...
  if (['/bypassed', '/navigate'].includes(location.pathname))
    location.assign(
      `https://universal-bypass.org${location.pathname}${location.search}`
    )
})

domainBypass('acortame.xyz', () => {
  if (window.location.hash)
    unsafelyNavigate(atob(window.location.hash.substr(1)))
})
...
ensureDomLoaded(() => {
  ...
  domainBypass(/linkvertise\.(com|net)|link-to\.net/, () =>
    insertInfoBox(
      FAST_FORWARD_INTERNAL_VERSION >= 9
        ? '{{msg.infoLinkvertise}}'
  ...
  domainBypass(/za\.(gl|uy)/, () => {
    ifElement('form#link-view', form => {
      ...
      document.querySelector('input[name="givenX"]').value =
        'VFl0utOEF6a7BiS8YJdqTg=='
      document.querySelector('input[name="givenY"]').value =
        'rsW06vBB1oIFVpnFz61t5Q=='
      form.submit()
      return
    })
      ...
      () => {
        window.setTimeout(() => {
          let payload = new URLSearchParams(
            new FormData(document.querySelector('#go-link'))
          ).toString()
  ...
  domainBypass(/oxy\.st|healthyteeth\.tips/, () => {
  ...
  domainBypass(/(wikitrik|linkerload).com/, () =>
    document
      .querySelector(
  ...
  domainBypass(
    ...
    () => {
      ...
      let u = aesCrypto.decrypt(
        convertfn(location.href.substr(location.href.indexOf('?o=') + 3)),
        convertfn('root')
      )
      ...
      if (isGoodLink(u)) {
        ...
        u = aesCrypto.decrypt(
          convertfn(uri.substr(uri.indexOf('?o=') + 3)),
          convertfn('root')
        )
        safelyNavigate(u)
        ...
        u = aesCrypto.decrypt(
          convertfn(location.href.substr(location.href.indexOf('#go') + 3)),
          convertfn('root')
        )
        location.hash = ''
  ...
  domainBypass('mlwbd.pw', () => {
    document.querySelectorAll("tr[id^='mov']").forEach(t => {
      ...
      t.querySelector("a[href^='javascript']").href =
        t.querySelector("input[name='FU']").value
    })
  })
  ...
  domainBypass('gaminplay.com', () => {
    if (
      window.location.href.substr(0, 31) == 'https://gaminplay.com/verify/?/'
    ) {
    ...
    awaitElement('#go-link input[name=csrf]', function () {
      ...
      document.querySelectorAll('script').forEach(script => {
        ...
        if (
          ...
          var data =
            '_a=' + matches[1] + '&_t=' + matches1[1] + '&_d=' + matches2[1]
          $.post('https://aylink.co/get/tk', data, function (data1) {
            $.post(
              'https://aylink.co/links/go2',
              'alias=' + alias + '&csrf=' + csrf + '&tkn=' + data1.th,
              function (data) {
  ...
  hrefBypass(
    /((psarips\.(com|net|org|eu|in|one|xyz|uk))|(psa\.(one|pm)))\/exit\//,
    () =>
  ...
  domainBypass(/tinyurl\.(is|so)/, () =>
    ifElement('a[id^="newskip-btn"][href]', safelyAssign)
  )
  ...
  domainBypass('noon.khsm.io', () => {
    const url = document
      .getElementsByClassName('download-link')[0]
      .getAttribute('href')
    safelyNavigate(url)
  })
  domainBypass('linkspy.cc', () => {
    const url = document
      .getElementsByClassName('skipButton')[0]
      .getAttribute('href')
    safelyNavigate(url)
  })
  domainBypass(/mflixpo\.xyz|themflix\.xyz/, () => {
    document.getElementsByClassName(
      'timed-content-client_hide_0_05_0'
    )[0].style = 'display: none;'
    document.getElementsByClassName(
      'timed-content-client_show_0_05_0'
    )[0].style = ''
  })
  ...
  domainBypass(/(indiainfo4u.in|intercelestial.com)/, () => {
    awaitElement('#landing', () => {
      document
        .querySelector(
          '#content > div.container > div > center:nth-child(1) > form'
        )
        .submit()
    })
    awaitElement('#pleasewait', () => {
      ...
      overrideScript.innerText =
        "var soramode = 'default';var threshold = 0;setTimeout(function(){jQuery('.to').show();jQuery('.wait').hide();},threshold);"
  ...
  domainBypass('ytsubme.com', () => {
    const url = document
      .querySelector(
        '#msg-box3-o > div > div > div.col-md-4.text-center > div > div > div > div.col-md-8.col-md-offset-2.text-xs-center > div > div > script'
      )
      .innerHTML.split(';')[7]
      .split('=')[1]
      .replaceAll("'", '')
  ...
  //Insertion point for domain-or-href-specific bypasses running after the DOM is loaded. Bypasses here will no longer need to call ensureDomLoaded.
  domainBypass('chooyomi.com', () =>
    awaitElement('.get-link > a:nth-child(1)', a => safelyNavigate(a.href))
  )
        ...
        if (a.firstChild.tagName == 'IMG') {
          ...
          a.href +=
            ...
            a.parentNode.textContent
              ...
              .replace(/[^a-zA-Z0-9]/g, '')
              .toLowerCase() +
            a.textContent.split(' ').join('').toLowerCase()
        }
      })
  )
  ...
  domainBypass(/(masteredutech|codecomets)\.com/, () =>
  ...
  domainBypass(/(techynroll|threadbolts|techitease)\.com/, () =>
  ...
  // mmcryptos domains
  domainBypass(
    /clickscoin\.com|dogeclick\.net|sl\.mcmfaucets\.xyz|short\.mcmcryptos\.xyz/,
  ...
  domainBypass(/exey\.io/, () =>
    ifElement('button.btn.btn-primary.btn-goo', a => a.click())
  )
  ...
  domainBypass('fc-lc.com', () =>
    awaitElement('.g-recaptcha.btn.btn-primary', b => b.click())
  )
  ...
  domainBypass(/cb\.(run|click)/, () =>
    ifElement('a.btn', a => safelyAssign(a.href))
  )
  ...
  let dT = setInterval(() => {
    ...
    if (typeof app_vars == 'object') {
      ...
      let iT = setInterval(() => {
        if (
          ...
          if (t == '') {
            if (
              FAST_FORWARD_INTERNAL_VERSION >= 10 &&
    ...
    if (typeof appurl == 'string' && typeof token == 'string') {
      const regex =
        /var count = [0-9]*;var countdown = setInterval\(function\(\){\$\("[a-z\-.# ]+"\)(\.attr\("href","#pleasewait"\))?(\.attr\("disabled",""\))?\.html\(count( \+ ".+")?\);if \(count < 1\) {clearInterval\(countdown\);(\$\("[a-z\-.# ]+"\)\.attr\("href",|window\.location=)"(https?:\/\/[^"]+)"( \+ hash)?\)?(\.removeAttr\("disabled"\)\.removeClass\("disabled"\))?(\.html\(".+"\))?;}count--;}, 1000\);/
